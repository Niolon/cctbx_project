"""Test NoMoRe with frequency refinement using perturbed starting scales."""
...
def test():
  """Test NoMoRe with frequency refinement."""
  print("=" * 60)
  print("NoMoRe Frequency Refinement Test (Perturbed Starting Scales)")
  print("=" * 60)
  ...
  # Create constraint WITH frequency refinement (using new defaults)
  # min_freq=0.1 (clamping), acoustic_init=10.0 (init acoustic modes)
  print("\n2. Creating constraint (refine_frequencies=True)...")
  ...
  constraint = nomore_adp.create(
    ...
    refine_frequencies=True  # Enable frequency refinement (only LOW modes)
  )

  print(f"  LOW modes: {len(constraint.low_indices)}")
  print(f"  MEDIUM modes: {len(constraint.medium_indices)}")

  # Set up flags - ADPs strictly follow from frequencies (freq → ADP relationship)
  # NO independent U* refinement - only frequency parameters are refined
  print("\n3. Setting refinement flags...")
  for sc in xs.scatterers():
    sc.flags.set_grad_site(False)
    sc.flags.set_grad_occupancy(False)
    if sc.flags.use_u_aniso():
      sc.flags.set_grad_u_aniso(False)  # ADPs come from frequencies, not refined directly
  ...
  # Enable Jacobian for frequency refinement
  constraint.enable_jacobian()
  constraint.update_frequencies()

  # Skip perturbation for now - just test if refinement works
  print("\n5. Initial frequency scales (all at 1.0)...")
  for i, param in enumerate(constraint.low_freq_params):
    if param.is_variable:
      print(f"    Mode {constraint.low_indices[i]}: scale = {param.value:.2f}")
  if constraint.mfsf_param and constraint.mfsf_param.is_variable:
    print(f"    MFSF: scale = {constraint.mfsf_param.value:.2f}")

  # Update C++ with current frequencies
  constraint.update_frequencies()
  ...
  ls_engine.build_up()
  print(f"  Initial chi²: {ls_engine.objective():.6f}")
  ...
  print(f"  chi² after cycle: {ls_engine.objective():.6f}")

  # Show final scales
  print("\n8. Final frequency scales:")
  for i, param in enumerate(constraint.low_freq_params):
    if param.is_variable:
      freq = constraint.frequencies[constraint.low_indices[i]]
      print(f"    Mode {constraint.low_indices[i]}: scale = {param.value:.4f} (freq = {freq*param.value:.2f} cm⁻¹)")
